...
class setting_config:
    ...
    model_config = {
        ...
        # ARL-UNet Base channel schedule (see research-report.md); used when network='arl_unet'
        ...
        # ARL-UNet ablation switches (train.py --disable_* overrides)
        'arl_cfg': {
            'use_artifact_stem': True,
            'use_robust_encoder': True,
            'use_skip_fusion': True,
            # soft: new default (artifact as hint); hard: legacy (1 - artifact_map) suppression
    ...
    direct_aug_p = 0.10
    direct_aug_artifacts = ('hair', 'air_bubble', 'skin_line', 'highlight')
    direct_aug_severities = (1, 2)
    artifact_consistency_p = 0.15
    artifact_consistency_artifacts = ('hair', 'air_bubble', 'skin_line', 'highlight')
    artifact_consistency_severities = (1,)
    artifact_consistency_lambda = 0.2
    artifact_consistency_artifact_lambda = 0.1
    artifact_consistency_downweight_beta = 0.7
    artifact_consistency_epochs = 320
    curriculum_epochs = 320
    curriculum_scheduler = 'ArtifactCurriculumLR'
    curriculum_warm_up_epochs = 20
    curriculum_eta_min = 1e-5
    artifact_val_benchmark = ''
    artifact_val_interval = 1

    work_dir = f'results/{network}_{datasets}_{datetime.now().strftime("%Y%m%d-%H%M%S")}/'
    ...
    opt = 'AdamW'
    assert opt in ['Adadelta', 'Adagrad', 'Adam', 'AdamW', 
                   'Adamax', 'ASGD', 'RMSprop', 'Rprop', 'SGD'], f'Unsupported optimizer! {opt}'
    if opt == 'Adadelta':
