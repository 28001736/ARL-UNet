...
class Dynamic_Feature_Fusion_Module(nn.Module):
    def __init__(self, dim_in):
        ...
        self.conv_att = nn.Sequential(
            nn.Conv2d(dim_in * 2, dim_in * 2, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.conv_rec = nn.Conv2d(dim_in * 2, dim_in, kernel_size=1, bias=False)
        self.shared_conv = nn.Conv2d(dim_in, 1, kernel_size=1, stride=1, bias=True)

    def forward(self, en_x, de_x, mask):
        mask = F.gelu(F.group_norm(mask, 1))
        output = torch.cat([en_x, de_x], dim=1)

        att = self.conv_att(self.avg_pool(output))
        output = output * att
        output = F.gelu(self.conv_rec(output))

        att = self.shared_conv(en_x) + self.shared_conv(de_x) + mask
...
class EMAUNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, c_list=[8, 16, 24, 32, 48, 64], bridge=True,
        ...
        if self.deep_supervision:
            ...
            if self.logger is not None:
                self.logger.info('Deep Supervision was used')
    ...
    def _build_ffl_adapters(self, c_list, ff_parser_cfg):
        self.ffl_s8 = nn.Identity()
        self.ffl_s16 = nn.Identity()
        self.ffl_s32 = nn.Identity()
        ...
        if isinstance(scales, str):
            scales = [scale.strip() for scale in scales.split(',') if scale.strip()]
        scales = set(scales)
        ...
        if 's8' in scales:
            self.ffl_s8 = FFParserLite(c_list[2], shared_radial=shared_radial, **common_kwargs)
        if 's16' in scales:
            self.ffl_s16 = FFParserLite(c_list[3], shared_radial=shared_radial, **common_kwargs)
        if 's32' in scales:
            self.ffl_s32 = FFParserLite(c_list[4], shared_radial=shared_radial, **common_kwargs)

        if self.logger is not None:
            self.logger.info(f'FF-Parser-Lite was used on scales: {sorted(scales)}')
    ...
    def forward(self, x):
        ...
        out4 = F.gelu(F.interpolate(self.dbn2(self.decoder2(out5)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))  # b, c3, H/17---16, W/17---16
        ...
        out4 = torch.add(out4, t4)  # b, c3, H/17---16, W/17---16
